...
class AppletComms(object):

    #Initialize the items required for IP Comms
    def __init__(self):
        self.ipAddress = '192.168.15.15' #'192.168.15.15' #IP Address of the RPi
    ...
    def writeApp(self, message):
        try:
            #Make sure there is a connection first before sending
            if (self.isEstablished):
                message = message + "\n"
                self.client.send(message) #self.clientAddr)
                return
